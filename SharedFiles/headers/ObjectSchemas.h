...
/* The maximum length a path can be before it is deemed to be
not-null-terminated, and thus invalid */
...
/* The rate in Hz that the Conduit driver and lib poll for updates from
 * eachother */
inline const double POLL_RATE = 2048.0;

/* The number of microseconds the client and driver should wait for the commit
 * flag of the other before timing out */
...
/**
 * @brief Represents the central header in shared memory, containing critical
 * metadata needed by both the Conduit lib and Driver, often simultaneously
 */
struct SharedMemoryHeader {
  ...
  /**
   * @brief The current version of the protocol used to ensure agreement on
   * reading and writing rules between the driver and lib
   */
  ...
  /** @brief The offset in bytes from the start of the shared memory to the
   * start of the path table */
  ...
  /**
   * @brief An offset in bytes (from <pathTableStart>) for where the driver is
   * currently writing a path, or 0 if the driver is not currently writing
   */
  ...
  /** @brief The offset in bytes from the start of the shared memory to the
   * start of the driver-client lane */
  ...
  /**
   * @brief The total number of writes made by the driver to the driver-client
   * lane, which is compared to a local read count by the lib to decide when new
   * packets can be read
   */
  std::atomic<uint64_t> driverClientWriteCount;

  /** @brief The current offset in bytes that the driver is writing to the
   * driver-client lane at */
  std::atomic<uint32_t> driverClientWriteOffset;

  /** @brief The current offset in bytes that the lib has read at from the
   * driver-client lane */
  ...
  /** @brief The offset in bytes from the start of the shared memory to the
   * start of the client-driver lane */
  ...
  /**
   * @brief The total number of writes made by the lib to the client-driver
   * lane, which is compared to a local read count by the driver to decide when
   * new packets can be read
   */
  std::atomic<uint64_t> clientDriverWriteCount;

  /** @brief The current offset in bytes that the lib is writing to the
   * client-driver lane at */
  std::atomic<uint32_t> clientDriverWriteOffset;

  /** @brief The current offset in bytes that the driver has read at from the
   * client-driver lane */
  std::atomic<uint32_t> clientDriverReadOffset;
};

/**
 * @brief Represents the metadata for a single state snapshot in the
 * driver-client lane. Each ObjectEntry is immediately followed a <X>Serialized
 * struct matching the type, containing type specific information
 */
struct ObjectEntry {
  /** @brief Used to check if the packet is aligned as expected in shared memory
   * to the read header */
  ...
  /** @brief True if this object is currently active/valid, false if it should
   * be removed */
  bool valid;

  /** @brief True/1 if this object has successfully and fully finish writing to
   * shared memory */
  alignas(64) std::atomic<uint64_t> committed;
};

/**
 * @brief Serialized representation of a device pose, containing both the
 * current and overwritten pose values
 */
...
/**
 * @brief Serialized representation of a boolean input, containing both the
 * current and overwritten values
 */
...
/**
 * @brief Serialized representation of a scalar input, containing both the
 * current and overwritten values
 */
...
/**
 * @brief Serialized representation of a skeleton input, containing both the
 * current and overwritten values
 */
...
/**
 * @brief Serialized representation of a pose input, containing both the current
 * and overwritten values
 */
...
/**
 * @brief Serialized representation of an eye tracking input, containing both
 * the current and overwritten values
 */
...
/**
 * @brief Header for client-to-driver command packets, containing metadata about
 * the command
 */
struct ClientCommandHeader {
  ...
  /** @brief Indicates whether the command has been fully written and is ready
   * to be read */
  alignas(64) std::atomic<uint64_t> committed;
};
...
struct CommandParams_SetUseOverriddenStateDevicePose {
  /** @brief Whether to use the overridden pose state instead of the actual
   * device pose */
...
struct CommandParams_SetUseOverriddenStateDeviceInput {
  /** @brief Whether to use the overridden input state instead of the actual
   * device input */
  bool useOverriddenState;
  /** @brief Offset into the path table identifying the target input */
  uint32_t inputPathOffset;
...
/**
 * @brief Parsed data from an ObjectEntry, used for processing after reading
 * from shared memory
 */
...
/**
 * @brief Parsed data from a ClientCommandHeader, used for processing after
...
/**
 * @brief Base struct for model objects that tracks whether an override state is
 * active
 */
struct ModelObjectState {
  /** @brief Whether to use the overridden state instead of the actual device
   * state */
  bool useOverriddenState;
};

/**
 * @brief Model representation of a serialized device pose with override state
 * tracking
 */
...
/**
 * @brief Model representation of a serialized boolean input with override state
 * tracking
 */
struct ModelDeviceInputBooleanSerialized : public ModelObjectState {
  /** @brief The serialized boolean data containing current and overwritten
   * values */
  DeviceInputBooleanSerialized data;
};

/**
 * @brief Model representation of a serialized scalar input with override state
 * tracking
 */
struct ModelDeviceInputScalarSerialized : public ModelObjectState {
  /** @brief The serialized scalar data containing current and overwritten
   * values */
  DeviceInputScalarSerialized data;
};

/**
 * @brief Model representation of a serialized skeleton input with override
 * state tracking
 */
struct ModelDeviceInputSkeletonSerialized : public ModelObjectState {
  /** @brief The serialized skeleton data containing current and overwritten
   * values */
  DeviceInputSkeletonSerialized data;
};

/**
 * @brief Model representation of a serialized pose input with override state
 * tracking
 */
...
/**
 * @brief Model representation of a serialized eye tracking input with override
 * state tracking
 */
struct ModelDeviceInputEyeTrackingSerialized : public ModelObjectState {
  /** @brief The serialized eye tracking data containing current and overwritten
   * values */
